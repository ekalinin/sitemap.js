...
import { createWriteStream, createReadStream, promises, WriteStream } from 'fs';
...
export const simpleSitemapAndIndex = async ({
  ...
  const sitemapAndIndexStream = new SitemapAndIndexStream({
    ...
    getSitemapStream: (i) => {
      ...
      try {
        ...
        throw new Error(
          `Failed to construct sitemap URL for index ${i}: ${
            err instanceof Error ? err.message : String(err)
          }`
        );
      }
  ...
  if (typeof sourceData === 'string') {
    try {
      ...
      throw new Error(
        `Failed to read sourceData file "${sourceData}": ${
          err instanceof Error ? err.message : String(err)
        }`
      );
    }
