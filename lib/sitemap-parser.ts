...
const tagTemplate: SitemapItemOptions = {
  img: [],
  video: [],
  links: [],
  url: ''
};
const videoTemplate: IVideoItem = {
  tag: [],
...
export async function parseSitemap (xml: Readable): Promise<ISitemapOptions> {
  ...
  saxStream.on('closetag', (tag): void => {
    switch (tag) {
      case 'url':
        ...
        currentItem = { ...tagTemplate, video: [], img: [], links: [] }
        break;
      case "video:video":
        currentItem.video.push(currentVideo)
        currentVideo = { ...videoTemplate, tag: [] }
        break;
      case "image:image":
  ...
  return new Promise((resolve, reject): void => {
    ...
    xml.pipe(saxStream)
    saxStream.on('error', (error: Error): void => {
      reject(error)
    })
  })
}
