    ...
    expect((await streamToPromise(smis)).toString()).toBe(
      el('url',
        ...
        el('video:video',
          ...
          `<video:gallery_loc title="${simpleText.slice(0, simpleText.length - escapable.length * 2) + attrEscaped + attrEscaped}">${simpleURLEscaped}</video:gallery_loc>` +
        ...
        el('news:news',
          el('news:publication',
            el('news:name', simpleTextEscaped) +
            el('news:language', 'en')
          ) +
